...
A skill for creating new skills, updating existing ones, and iteratively improving them.
...
- **Concise over complete.** Less text that clearly describes what's meant is better than exhaustive coverage. Merge related points into single statements rather than enumerating them separately.
- **What and why, not how.** Explain the goal and the reasoning. Trust the model to figure out the mechanics — it's smart.
- **No restating the obvious.** If something is derivable from context or already implied, leave it out.
- **Prefer examples over rules.** A good example often conveys more than a paragraph of instruction.
- **Explain why, not just what.** In lieu of heavy-handed MUSTs, explain why something matters — that generalizes better than rigid rules.
